...
export class OptionCtrl {
  ...
  constructor($scope, $timeout) {
    ...
    this.bingClientId = '';
    //To display the words in the view
    this.wordDifficultyHash = JSON.parse('{}');
  ...
  getData() {
    chrome.storage.local.get(null, (data) => {
      ...
      var wordDifficultyHash = JSON.parse('{}');
      angular.forEach(this.difficultyBuckets, function(value, key) {
        angular.merge(wordDifficultyHash, value)
      });
      this.wordDifficultyHash = wordDifficultyHash;
  ...
  // TODO: on deleting doesn't automatically get updated
  ...
  removeWordFromDifficultyBucket(word){
    ...
    Object.keys(difficultyBuckets).forEach(function(key) {
      Object.keys(difficultyBuckets[key]).forEach(function(k) {
        if(k == word){
          delete difficultyBuckets[key][word];
        }
      });
    });

    //Deletes keys with empty values in hash
    Object.keys(difficultyBuckets).forEach(function(key) {
      if (Object.keys(difficultyBuckets[key]).length === 0 && difficultyBuckets[key].constructor === Object){
        delete difficultyBuckets[key];
      }
    });
